...
void DrawGVizColorDemo() {
  ...
  PNGraph NGraph1=TSnap::LoadEdgeList<PNGraph>("payment_graph_physician_company.csv", 0, 1);
 ...
 // Exts.Add("ps");
  ...
  for (TNGraph::TNodeI NI = NGraph1->BegNI(); NI < NGraph1->EndNI(); NI++) {
    //int RVal = TInt::Rnd.GetUniDevInt(0, Colors.Len()-1);
    if(NI.GetOutDeg() > 0) NIdColorH.AddDat(NI.GetId(), Colors.GetKey(0));
	else NIdColorH.AddDat(NI.GetId(), Colors.GetKey(1));
  }
  ...
  for (int i = 0; i < LNames.Len(); i++) {
    for (int e = 0; e < Exts.Len(); e++) {
      //for (int d = 0; d < 2; d++) {
        ...
        TStr FName = TStr::Fmt("%s/demo_%s_%s_col.%s", DIRNAME, "p_c_graph" , "sfdp", Exts[e].CStr());
        ...
        //if (d) {
          TSnap::DrawGViz(NGraph1, TGVizLayout(4), FName, "sfdp", true, NIdColorH);
        //}
        //else {
